...
const getAnalyticsRowData = async (
  ...
  while (total > 0) {
    ...
    let processedData = results.rows.map((row) => {
      return {
        ...row.reduce<Record<string, string | string[] | number | null>>(
          (acc, val, index) => {
  ...
  queryFn: async () => {
    const { organisationUnits, dataViewOrganisationUnits, displayName } =
      await getDHIS2Resource<{
        ...
        params: {
          fields:
            "displayName,organisationUnits[id,name,leaf,parent],dataViewOrganisationUnits[id,name,leaf,parent]",
        },
      });
    ...
    queryFn: async () => {
      ...
      if (currentProgram === undefined) {
        currentProgram = await getDHIS2Resource<Program>({
          ...
          params: {
            fields:
              "id,name,registration,programType,selectIncidentDatesInFuture,selectEnrollmentDatesInFuture,incidentDateLabel,enrollmentDateLabel,trackedEntityType[id],organisationUnits[id,name],programTrackedEntityAttributes[id,name,mandatory,valueType,displayInList,sortOrder,allowFutureDate,trackedEntityAttribute[id,name,generated,pattern,unique,valueType,orgunitScope,optionSetValue,displayFormName,optionSet[options[code,name]]]],programStages[id,name,repeatable,sortOrder,programStageDataElements[compulsory,dataElement[id,name,formName,optionSetValue,valueType,optionSet[options[code,name]]]]]",
          },
        });
...
export const rawDataQueryOptions = () => {
  return queryOptions({
    queryKey: ["rawData"],
    queryFn: async () => {
      const data: Array<Record<string, string | string[] | number | null>> =
...
export const trackedEntitiesQueryOptions = ({
  ...
  queryOptions({
    queryKey: ["programs", program, ou, registration, page, pageSize, ouMode],
    queryFn: async () => {
