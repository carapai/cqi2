  ...
  const processed: Array<InstanceDisplay> = trackedEntityInstances.map(
    ({ attributes, enrollments, ...rest }) => {
      ...
      orderBy(
        ...
        allEvents[`${index}e`] = dayjs(e.eventDate).format("YYYY-MM-DD");
      ...
      attributes
        .concat(enrollments.flatMap(({ attributes }) => attributes ?? []))
        .forEach(({ value, attribute }) => {
          if (attribute === "kHRn35W3Gq4") {
            ...
            attributeValues[attribute] = processedOptions[value] ?? "";
          } else {
...
export const relativePeriods2: { [key: string]: string[] } = {
  ...
  QUARTERS_THIS_YEAR: lastNPeriods((dayjs().quarter() - 1) * 3, "months", true),
};
...
export const computerIndicator = (
  ...
  const denominators = processedData.map((i) => Number(i["RgNQcLejbwX"] ?? 0));
  let value = NaN;
...
export const downloadLayered = async ({
  ...
  let availableData: Array<Record<string, string | string[] | number | null>> =
    await db.analytics.toArray();
  ...
  const data = structure.metaData.dimensions.ou.map((ou) => ({
    ...
    ...structure.metaData.dimensions.pe.reduce<Record<string, number | string>>(
      (acc, pe) => {
  ...
  if (counting === "units") {
    ...
    data = [
      ...
      ...indicators.map((indicator) => ({
        ...indicator,
        ...structure.metaData.dimensions.pe.reduce<Record<string, number>>(
          (acc, pe) => {
            const available = availableData.filter(
              (d) =>
                isArray(d.periods) &&
            ...
            acc[`${pe}_total`] = uniq(available.flatMap(({ ou }) => ou)).length;
      ...
      ...structure.metaData.dimensions.ou.map((ou) => ({
        ...
        ...structure.metaData.dimensions.pe.reduce<Record<string, number>>(
          (acc, pe) => {
            ...
            acc[`${pe}_total`] = uniq(available.flatMap(({ pi }) => pi)).length;
            acc[`${pe}_running`] = uniq(
